...
function watchDeep(source, cb, options) {
  return watch(source, cb, __spreadProps$5(__spreadValues$5({}, options), {
    deep: true
  }));
}
...
function watchImmediate(source, cb, options) {
  return watch(source, cb, __spreadProps$3(__spreadValues$3({}, options), {
    immediate: true
  }));
}
...
function useFetch(url, ...args) {
  ...
  const execute = async (throwOnFailed = false) => {
    ...
    return new Promise((resolve, reject) => {
      var _a3;
      fetch(context.url, __spreadProps$42(__spreadValues$d(__spreadValues$d({}, defaultFetchOptions), context.options), {
...
function useImage(options, asyncStateOptions = {}) {
  const state = useAsyncState(() => loadImage(toValue(options)), void 0, __spreadValues$a2({
    resetOnExecute: true
  }, asyncStateOptions));
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  const state = reactive(useScroll(element, __spreadProps$32(__spreadValues$92({}, options), {
  ...
  watch(() => [state.arrivedState[direction], toValue(element)], checkAndLoad, { immediate: true });
  return {
    isLoading
  };
}
